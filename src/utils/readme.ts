...
- Scripts will have **source maps** enabled, beacuse of the \`--enable-source-maps\` flag in the ${
    packageJson.scripts?.ev
      ? `\`start\` and \`dev\` scripts`
      : `\`start\` script`
  } and becuase the generated \`${
    args.buildTool
  }\` config is set to generate inline source maps.
...
However, for cleaner code, the \`--modules-resolution=node\` flag has been given to the \`start\` ${
        packageJson.scripts?.dev ? 'and `dev` scripts' : 'script'
      }. In short, this tells Node to try and append a \`.js\` extension to imports.
`
    } else {
